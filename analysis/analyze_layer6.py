"""
Systematic Analysis of Layer 6 (Existential-Philosophical Representation) Results
==================================================================================

This script performs comprehensive analysis of Layer 6 evaluation data.
...
df = load_layer_data(layer=6)

print("=" * 100)
print("LAYER 6 (EXISTENTIAL-PHILOSOPHICAL REPRESENTATION) - SYSTEMATIC ANALYSIS")
print("=" * 100)
...
dimensions = ['LP', 'MR', 'HC', 'ME']
dim_names = {
    'LP': 'Life Philosophy',
    'MR': 'Moral Reflection',
    'HC': 'Human Condition',
    'ME': 'Meaning Exploration'
}
...
# ========================================
# 11. CROSS-LAYER COMPARISON (Layer 4 vs Layer 5 vs Layer 6)
# ========================================
print("\n" + "=" * 100)
print("11. CROSS-LAYER COMPARISON (Layer 4 vs Layer 5 vs Layer 6)")
print("=" * 100)

# Load Layer 4 and Layer 5 data for comparison
df4 = load_layer_data(layer=4)
df5 = load_layer_data(layer=5)

layer_pairs = [("Layer 4", "Layer 5", df4, df5), ("Layer 4", "Layer 6", df4, df), ("Layer 5", "Layer 6", df5, df)]

for mode in ['content-limit', 'title-limit']:
    print(f"\n{mode.upper()}:")
    for name_a, name_b, df_a, df_b in layer_pairs:
        da = df_a[df_a['mode'] == mode][['story_id', 'iterative_overall_score']].rename(
            columns={'iterative_overall_score': 'score_a'}
        )
        db = df_b[df_b['mode'] == mode][['story_id', 'iterative_overall_score']].rename(
            columns={'iterative_overall_score': 'score_b'}
        )
        merged_layers = da.merge(db, on='story_id')

        r, p_val = stats.pearsonr(merged_layers['score_a'], merged_layers['score_b'])
        t_stat, p_val_t = stats.ttest_rel(merged_layers['score_a'], merged_layers['score_b'])

        print(f"\n  {name_a} vs {name_b}:")
        print(f"    Pearson correlation: r={r:.3f}, p={p_val:.4f}")
        print(f"    {name_a} mean: {merged_layers['score_a'].mean():.3f}")
        print(f"    {name_b} mean: {merged_layers['score_b'].mean():.3f}")
