"""
Layer 5: Emotional Resonance Layer (情感层)

Implements emotional dimension evaluation using LLM-based judging.

Dimensions:
- Emotional Depth (情感深度)
- Character Empathy (角色共情)
- Emotional Authenticity (情感真实性)
- Reader Impact (读者冲击力)
"""
...
from ..llm.emotional_judge import EmotionalJudge


class Layer5Emotional(BaseLayer):
    """
    Layer 5: Emotional Resonance Evaluator

    Uses LLM-based judging to assess emotional depth, empathy,
    authenticity, and lasting impact of narratives.
    ...
    def __init__(self, config: Dict[str, Any], llm_client: Any):
        """
        Initialize Layer 5 with configuration and LLM client

        Args:
            config: Layer 5 configuration from YAML
                {
                    "enabled": True,
                    "temperature": 0.0,  # 0.0 for deterministic (consistency study)
                    "prompt_version": "v1",  # "v1" to "v5" for multi-prompt study
                    "dimensions": {
                        "emotional_depth": {"enabled": True, "weight": 0.30},
                        "character_empathy": {"enabled": True, "weight": 0.25},
                        "emotional_authenticity": {"enabled": True, "weight": 0.25},
                        "reader_impact": {"enabled": True, "weight": 0.20}
                    }
                }
            llm_client: LLM客户端实例
        """
        ...
        # 调用父类初始化
        super().__init__(LayerType.EMOTIONAL, config)

        # 初始化Judge
        self.judge = EmotionalJudge(
    ...
    def _validate_config(self) -> None:
        ...
        if "enabled" not in self.config:
            raise ValueError("Layer 5 config must have 'enabled' field")

        if "dimensions" not in self.config:
            raise ValueError("Layer 5 config missing 'dimensions' section")
        ...
        if not has_enabled:
            raise ValueError("Layer 5 must have at least one dimension enabled")

    def evaluate(self, text: str, context: Optional[Dict[str, Any]] = None) -> LayerResult:
        """
        Evaluate emotional dimension of text using LLM judge
        ...
        Returns:
            LayerResult with emotional scores and LLM rationale
        """
        ...
        try:
            ...
            if judge_result.get("error"):
                return LayerResult(
                    layer_num=5,
                    layer_name="Emotional Layer (情感层)",
                    score=0.0,
            ...
            return LayerResult(
                ...
                layer_name="Emotional Layer (情感层)",
                score=0.0,
                error=f"Layer 5 evaluation failed: {str(e)}",
                computation_time=time.time() - start_time
            )
